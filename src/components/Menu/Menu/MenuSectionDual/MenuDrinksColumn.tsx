...
interface MenuDrinksColumnProps {
  drinksData: MenuItem[];
  selectedDrink: MenuItem;
  setSelectedDrink: Dispatch<SetStateAction<MenuItem>>;
  showAllDrinks: boolean;
  setShowAllDrinks: (show: boolean) => void;
}

export function MenuDrinksColumn({
  drinksData,
  selectedDrink,
  setSelectedDrink,
  showAllDrinks,
  setShowAllDrinks,
}: MenuDrinksColumnProps) {
  // Referência para o topo desta coluna/seção
  const columnRef = useRef<HTMLDivElement>(null);

  const displayedDrinks = showAllDrinks ? drinksData : drinksData.slice(0, 3);
  const hasMoreDrinks = drinksData.length > 3;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 flex-row-reverse">
        ...
        <h2 className="text-3xl sm:text-4xl font-medium text-text-title tracking-tight text-right">
          Drinks Autorais
        </h2>
      </div>
      ...
      {hasMoreDrinks && (
        <div>
          ...
          >
            <span>
              {showAllDrinks ? "Ver menos drinks" : "Ver mais drinks"}
            </span>
            {showAllDrinks ? (
