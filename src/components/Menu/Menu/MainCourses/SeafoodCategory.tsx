...
import { SEAFOOD_DATA } from "@/src/constants/seafoodData";

export function SeafoodCategory() {
  const [selectedDish, setSelectedDish] = useState(SEAFOOD_DATA[0]);
  const [showAll, setShowAll] = useState(false);

  const displayedDishes = showAll ? SEAFOOD_DATA : SEAFOOD_DATA.slice(0, 3);
  const hasMore = SEAFOOD_DATA.length > 3;

  return (
    <div className="w-full">
      {/* DESKTOP: Do Mar (Zig-zag invertido: Imagem à esquerda, Lista à direita) */}
      <div className="hidden lg:grid grid-cols-12 gap-16 xl:gap-24 items-start">
        {/* Lado Esquerdo: Imagem Fixa Proporcional */}
        ...
        {/* Lado Direito: Cabeçalho da Categoria + Lista de Pratos */}
      ...
      {/* MOBILE: Do Mar */}
      <div className="lg:hidden flex flex-col space-y-8 mt-12">
        <span className="font-sans text-xs uppercase tracking-widest text-primary font-semibold">
          Do Mar à Mesa
        </span>
        {displayedDishes.map((dish) => (
